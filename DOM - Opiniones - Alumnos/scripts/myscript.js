...
const $d = document,
  ...
  $imgUsuario = $d.querySelector(".usuario-imagen"),
  $nomeUsuario = $d.querySelector(".usuario-nombre"),
  $traballoUsuario = $d.querySelector(".usuario-trabajo");

  let posicion = 0;

  function novaOpinion(posicion) {
    $opinion.innerHTML = opiniones[posicion].texto;
    $imgUsuario.src = opiniones[posicion].imagen;
    $nomeUsuario.innerHTML = opiniones[posicion].nombre;
    $traballoUsuario.innerHTML = opiniones[posicion].trabajo;
  }

  $opiniones.addEventListener("click", (e) => {
    e.preventDefault();
    // Con clearInterval limpiamos o intervalo e poñemos a animación a cero
    clearInterval(id)
    novaOpinion(posicion)
    console.log($barra.style.animationName)
    $barra.style.animationName="none"
    //$barra.style.animationName="grow"
    // animación relacionada co CSS, como está arriba non funciona, hay que
    // poñer un intervalo de tempo
    setTimeout(()=>$barra.style.animationName="grow",0) 
